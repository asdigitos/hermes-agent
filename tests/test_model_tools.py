...
def test_registry_generation_tolerates_mixed_version_registry(monkeypatch):
    """Older in-memory registries may not yet expose ``_generation``."""
    import model_tools

    monkeypatch.delattr(model_tools.registry, "_generation", raising=False)
    assert _registry_generation() == 0
