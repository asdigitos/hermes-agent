...
def get_env_value(name: str, default=None):
    ...
    Resolve :func:`hermes_cli.config.get_env_value` at call time so imports that
    happen while tests temporarily monkeypatch that helper do not freeze a stale
    function into this module for the rest of the process. Tests can still patch
    ``tools.xai_http.get_env_value`` directly to inject dotenv-only secrets into
    the xAI credential resolver.
    """
    try:
